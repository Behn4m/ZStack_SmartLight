...
void interrupt_OCC_sensor(void);
...
{                                                                 
  uint8 x = 0;
  uint32_t ui32DirPins = ~GPIOPinRead(BSP_KEY_BASE, BSP_KEY_ALL);

    if(ui32DirPins & BSP_KEY_NWK_START)
  {
    x |= HAL_KEY_SW_1;
  } 
...
//  halIntKeys = hal_key_int_keys();
  uint32_t ui32OCCPin = ~GPIOPinRead(BSP_OCCSENSOR_BASE, BSP_OCCSENSOR);  
  if(ui32OCCPin)
  {
   halIntKeys = HAL_OCCSENSOR;
...
void HalKeyConfig( bool interruptEnable, halKeyCBack_t cback)
{
  //farhad
    ...
    GPIOPinTypeGPIOInput(BSP_KEY_BASE, BSP_KEY_ALL );
    IOCPadConfigSet(BSP_KEY_BASE,BSP_KEY_ALL, IOC_OVERRIDE_PUE);
    
    GPIOPinTypeGPIOInput(BSP_OCCSENSOR_BASE, BSP_OCCSENSOR );
    IOCPadConfigSet(BSP_OCCSENSOR_BASE,BSP_OCCSENSOR, IOC_OVERRIDE_PUE);
    
    // Disable interrupts
    GPIOPinIntDisable(BSP_KEY_BASE,BSP_KEY_ALL);
    GPIOPinIntDisable(BSP_OCCSENSOR_BASE,BSP_OCCSENSOR);
    ...
    GPIOPinIntEnable(BSP_OCCSENSOR_BASE,BSP_OCCSENSOR);
...
uint8 hal_key_no_debounce(void)
{
  ...
  dirPins = (~GPIOPinRead(BSP_KEY_BASE,
                          BSP_KEY_ALL)) \
                          & BSP_KEY_ALL;
  if(dirPins & BSP_KEY_Light_1)
  {
    x |= HAL_KEY_SW_1;
  }
  if(dirPins & BSP_KEY_Light_2)
  {
    x |= HAL_KEY_SW_2;
  }
